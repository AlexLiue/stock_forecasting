...
def plot_skdj_avg(df):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(x=df["trade_date"], y=df["avg"], name="AVG", xaxis="x", yaxis="y1")
    )

    fig.add_trace(
        go.Scatter(x=df["trade_date"], y=df["DIFF"], name="DIFF", xaxis="x", yaxis="y2")
    )
    fig.add_trace(
        go.Scatter(x=df["trade_date"], y=df["DEA"], name="DEA", xaxis="x", yaxis="y2")
    )
    fig.add_trace(
        go.Scatter(x=df["trade_date"], y=df["MACD"], name="MACD", xaxis="x", yaxis="y2")
    )

    fig.add_trace(
        go.Scatter(
            x=df["trade_date"], y=df["SKDJ_K"], name="SKDJ", xaxis="x", yaxis="y3"
        )
    )
    fig.add_trace(
        go.Scatter(
            x=df["trade_date"], y=df["SKDJ_D"], name="SKDJ", xaxis="x", yaxis="y3"
        )
    )
    ...
    fig.update_layout(
        title_text=f"{df.at[0, 'symbol']} N 日特征参数",
        title_x=0.5,
        margin=dict(l=10, r=10, b=40, t=40),
        xaxis=dict(domain=[0.08, 0.98], showline=True),
        yaxis1=dict(
            title=dict(text="SKDJ-( K / D 百分位)", standoff=10),
...
if __name__ == "__main__":
    ...
    features = get_features(
        cfg, engine, logger, "000001", start_date="20220701", end_date="20240501"
    )
