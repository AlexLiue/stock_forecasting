...
from forecasting.utils.utils import get_mock_connection, load_table, get_cfg, get_logger, timedelta_to_str, \
    get_query_condition
...
class Stock:
    ...
    def df(self):
        if self.period == 'day':
            condition = get_query_condition(symbol=self.symbol, start_date=self.start_date, end_date=self.end_date)
            sql = f"SELECT * FROM {self.cfg['mysql']['database']}.stock_daily_qfq t " \
                  f"WHERE {condition} ORDER BY symbol, trade_date"
            self.logger.info(f"Execute SQL  [{sql}]")
            return pd.read_sql(sql, self.engine)
        elif self.period == 'min30':
            condition = get_query_condition(symbol=self.symbol, start_date=self.start_date, end_date=self.end_date)
            sql = f"SELECT * FROM {self.cfg['mysql']['database']}.stock_min30_qfq t " \
                  f"WHERE {condition} ORDER BY symbol, trade_date, trade_time"
            self.logger.info(f"Execute SQL  [{sql}]")
            return pd.read_sql(sql, self.engine)
        else:
            self.logger.ERROR(f"Period[{self.period}] Error, Only support Period[day, min30]...")

    @LazyProperty
    def TRADE_DATE(self):
        TRADE_DATE = None
        if "trade_time" in self.df.columns:
            TRADE_DATE = pd.to_datetime(self.df['trade_date'].map(lambda d: str(d.strftime('%Y-%m-%d '))) +
                                        self.df['trade_time'].map(lambda t: timedelta_to_str(t)))
        else:
            TRADE_DATE = pd.to_datetime(self.df['trade_date'].map(lambda d: str(d.strftime('%Y-%m-%d '))) + "12:00:00")
    ...
    def plot(self):
        ...
        fig.add_trace(go.Scatter(x=dt, y=FT['DIFF_12_26_9'], name="DIFF_12_26_9", xaxis="x", yaxis='y2'))
        fig.add_trace(go.Scatter(x=dt, y=FT['DEA_12_26_9'], name="DEA_12_26_9", xaxis="x", yaxis='y2'))
        fig.add_trace(go.Scatter(x=dt, y=FT['MACD_12_26_9'], name="MACD_12_26_9", xaxis="x", yaxis='y2'))

        fig.add_trace(go.Scatter(x=dt, y=FT['SKDJ_K_36_5'], name="SKDJ_K_36_5", xaxis="x", yaxis='y3'))
        fig.add_trace(go.Scatter(x=dt, y=FT['SKDJ_D_36_5'], name="SKDJ_D_36_5", xaxis="x", yaxis='y3'))
        ...
        dt_breaks = [d.strftime('%Y-%m-%d %H:%M:%S') for d in dt_all if d.strftime('%Y-%m-%d %H:%M:%S') not in dt_date]
