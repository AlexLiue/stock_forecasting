...
def sync(drop_exist=False):
    ...
    try:
        start_date = query_last_api_sync_date('stock_hk_ggt_components_em', 'stock_hk_ggt_components_em')
        ...
        if start_date < end_date:
            ...
            logger.info(f"Table [stock_hk_ggt_components_em] Early Synced start_date[{start_date}] end_date[{end_date}], Skip ...")
    except Exception:
        logger.error(f"Table [stock_hk_ggt_components_em] SyncFailed", exc_info=True)
        update_sync_log_state_to_failed('stock_hk_ggt_components_em', 'stock_hk_ggt_components_em')
