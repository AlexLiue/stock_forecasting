...
def query_last_sync_info(trade_code, engine, logger):
    ...
    """
    result = []
    query_last_date = f"SELECT NVL(MAX(\"时间\"), TO_DATE('1970-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS')) as max_date FROM STOCK_ZH_A_HIST_30MIN_QFQ WHERE \"股票代码\"='{trade_code}'"
    ...
    try:
        ...
        for row_idx in range(trade_code_list.shape[0]):
            ...
            if start_date < end_date:
                logger.info( f"Execute Sync stock_zh_a_hist_30min_qfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}]")
                ...
                if not df.empty:
                    ...
                    if last_sync_close is None or df.loc[df["时间"]==start_date, "收盘"][0] == last_sync_close:
                        ...
                        save_to_database(df, "stock_zh_a_hist_30min_qfq", engine, index=False, if_exists='append', chunksize=20000)
                        ...
                        logger.info(f"Execute Sync stock_zh_a_hist_30min_qfq, Detect QFQ data updated, Clean History Data With SQL [{clean_sql}], Recall Sync")
                        exec_sql(clean_sql)
                ...
                logger.info(f"Execute Sync stock_zh_a_hist_30min_qfq  trade_code[{trade_code}] trade_name[{trade_name}] from [{start_date}] to [{end_date}], Skip Sync ... ")

        update_sync_log_date('stock_zh_a_hist_min_em', 'stock_zh_a_hist_30min_qfq', f'{str(end_date)}')

    except Exception:
        logger.error(f"Table [stock_zh_a_hist_30min_qfq] Sync  Failed", exc_info=True)
        update_sync_log_state_to_failed('stock_zh_a_hist_min_em', 'stock_zh_a_hist_30min_qfq')
