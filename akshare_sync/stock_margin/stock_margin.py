...
def sync(drop_exist=False):
    ...
    # 查询交易日历
    trade_date_sql = (
        f"SELECT trade_date FROM {db}.stock_trade_date WHERE exchange ='SSE';"
    )
    logger.info(f"Execute SQL  [{trade_date_sql}]")
    trade_date = pd.read_sql(trade_date_sql, engine)
    # 查询历史最大同步时间
    max_trade_date_sql = (
        f"SELECT IFNULL(MAX(trade_date),DATE('1990-01-01')) as trade_date "
    ...
    start_date = max(
        datetime.datetime.strptime("20150101", "%Y%m%d").date(), max_rade_date
    )
    end_date = datetime.datetime.now().date()

    trade_date = trade_date[
        (trade_date["trade_date"] > start_date) & (trade_date["trade_date"] < end_date)
    ]
    ...
    for index, row in trade_date.iterrows():
        ...
        logger.info(
            f"Execute Sync Exchange[SSE] TradeDate[{date}] By [stock_margin_detail_sse]"
        )
        ...
        if not df.empty:
            ...
            logger.info(
                f"Execute Sync Exchange[SSE] TradeDate[{date}] Write[{df.shape[0]}] Records"
            )

    # SSE 融资融券明细
    trade_date_sql = (
        f"SELECT trade_date FROM {db}.stock_trade_date WHERE exchange ='SZSE';"
    )
    logger.info(f"Execute SQL  [{trade_date_sql}]")
    trade_date = pd.read_sql(trade_date_sql, engine)
    # 查询历史最大同步时间
    ...
    for index, row in trade_date.iterrows():
        ...
        date = str(trade_date.strftime("%Y%m%d"))
        logger.info(
            f"Execute Sync Exchange[SZSE] TradeDate[{date}] By [stock_margin_detail_szse]"
        )
        ...
        if not df.empty:
            ...
            logger.info(
                f"Execute Sync Exchange[SZSE] TradeDate[{date}] Write[{df.shape[0]}] Records"
            )
