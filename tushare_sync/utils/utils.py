...
def get_ts_code_list(interval, ts_code_limit):
    ...
    while True:
        ...
        df_ts_code = ts_api.stock_basic(
            **{"limit": 1000, "offset": ts_code_offset}, fields=["ts_code"]
        )
        time.sleep(interval)
        if df_ts_code.last_valid_index() is not None:
            ...
            logger.info(
                "Query ts_code from tushare with api[stock_basic] from ts_code_offset[%d] ts_code_limit[%d]:"
                " Result[%s]"
                % (ts_code_offset, ts_code_limit, ts_code.str.cat(sep=","))
            )
...
def exec_sync_with_spec_date_column_v2(
    table_name,
    api_name,
    fields,
    date_column,
    ...
    while True:
        try:
            ...
            while step_start <= end:
                ...
                while True:
                    logger.info(
                        "Query [%s] from tushare with api[%s] %s[%s-%s]"
                        " from offset[%d] limit[%d]"
                        % (
                            table_name,
                            api_name,
